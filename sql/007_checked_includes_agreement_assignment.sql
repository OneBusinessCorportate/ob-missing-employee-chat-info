...
--
-- «Проверено», если по чату есть назначение в Agreements.
--
-- Задача: раньше checked = «есть присутствие ИЛИ сообщения» (реальные данные от
-- бота). Но у части активных чатов бот ещё не видел активности, хотя
-- ответственные по ним УЖЕ назначены в Agreements (mqa_chats.accountant /
-- .manager). Такие чаты показывались как «не проверено», хотя платформа о них
-- всё знает: has_accountant / has_manager у них и так true (сигнал agr уже
-- участвует в этих флагах после миграции 006). Логичнее не считать их
-- «не проверенными».
--
-- Поэтому расширяем ТОЛЬКО выражение checked: чат считается проверенным, если по
-- нему есть присутствие, сообщения ИЛИ назначение в Agreements (agr.a_acc /
-- agr.a_mgr — те же непустые/не-плейсхолдерные поля, что уже дают has_*). Прочая
-- логика вью, столбцы и доступ не меняются. Данные исходных таблиц не трогаем.
