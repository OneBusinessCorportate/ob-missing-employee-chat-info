-- Аддитивная, только на чтение миграция. Применяется к проекту OB FAQ
-- (fjsogozwseqoxgddjeig).
--
-- «Владелец чата» (менеджер) в отчёте и на дашборде.
--
-- Задача: делить проблемные чаты по менеджеру-владельцу — «Блок Ripsime»,
-- «Блок Shogher» и т. п., чтобы в Telegram-сводке первыми шли строки вида
-- «У Shogher проблемных чатов - 5 @Manageronebusiness».
--
-- Кто «владелец» чата: определяем по присутствию менеджеров в чате
-- (public.chat_employee_presence + public.employees, role='manager', is_active).
-- В большинстве клиентских чатов присутствует общий (служебный) аккаунт
-- «Manager OneBusiness» (@manager_onebusiness) — он есть почти везде и сам по
-- себе не различает клиентов. Поэтому владельцем считаем КОНКРЕТНОГО менеджера
-- (Ripsime, Shogher, …), а служебный аккаунт берём владельцем только когда
-- другого менеджера в чате нет. Если менеджеров нет вовсе — владельца нет
-- (такие чаты попадают в блок «без назначенного менеджера»).
--
-- @-упоминание берём из employees.telegram_username того же сотрудника, что и
-- владелец, — никаких новых таблиц/сопоставлений по тексту не заводим.
--
-- Меняем ТОЛЬКО определение вью; данные исходных таблиц не трогаем. Изменение
-- строго аддитивно: добавляются три столбца (owner_manager_id,
-- owner_manager_name, owner_manager_username), прежние столбцы не меняются.
