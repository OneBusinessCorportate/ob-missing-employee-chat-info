...
test("пустой HVHH считается «нет HVHH» даже при наличии номера договора", () => {
  const { noHvhh, counts } = computeClientChecks([
  ...
  // Номер договора больше НЕ трактуется как «HVHH есть» — проверяется реальное
  // поле hvhh, поэтому все три строки без HVHH попадают в список.
  assert.equal(counts.no_hvhh, 3);
  assert.deepEqual(
    noHvhh.map((r) => r.agr_no).sort(),
    ["B-4233", "SQA-VONABI", "ИП Гончар/ 4345 RU"],
  );
});
