...
// Нужно ли блокировать запрос. Блокируем ТОЛЬКО обычного бухгалтера
// (есть emp, adm=false) с непустым числом необработанных тикетов на защищённом
// пути. Админ (adm) и запросы без идентичности бухгалтера не блокируются.
export function gateDecision({ session, unanswered, path }) {
  ...
  if (!session.emp) return { blocked: false, reason: "no_accountant" };
