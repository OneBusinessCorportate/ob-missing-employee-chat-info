...
// Тестовые чаты (созданные QA, напр. «testchat1», «testchat67») не должны
// попадать в реальный чек-лист. По умолчанию исключаем всё, чьё название
// начинается с «test». Шаблон можно переопределить через env TEST_CHAT_PATTERN.
...
// opts.testPattern задаёт свой шаблон распознавания тестовых чатов.
export function computeProblems(data, opts = {}) {
  const { excludeTest = true, testPattern = DEFAULT_TEST_PATTERN } = opts;
  const allRows = data || [];

  // Сначала отсекаем тестовые чаты, чтобы они не влияли ни на список, ни на
  // счётчики (включая «всего чатов»).
...
// Опции исключения тестовых чатов берутся из окружения:
...
// Тестовые чаты исключаются (см. testOptsFromEnv).
export async function getProblemChats() {
  ...
  return computeProblems(data, testOptsFromEnv());
}
